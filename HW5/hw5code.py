...
def find_best_split(feature_vector, target_vector):
    ...
    * За наличие в функции циклов балл будет снижен. Векторизуйте! :)
    ...
    H_l = 2 * p_0_l * p_1_l
    H_r = 2 * p_0_r * p_1_r
    Q = -(H_l * l_lens + H_r * r_lens) / len(target_vector)
    best_ind = np.argmax(Q)
    return thresholds, Q, thresholds[best_ind], Q[best_ind]
...
class DecisionTree:
    ...
    def _fit_node(self, sub_X, sub_y, node):
        ...
        for feature in range(0, sub_X.shape[1]):
            ...
            n_left = (feature_vector < threshold).sum()
            if n_left == 0 or n_left == len(feature_vector):
                continue
